...
backup_download() {
	...
	wget --no-check-certificate -N "https://raw.githubusercontent.com/xxsisis/Others/master/shadowsocks-rust/v1.14.1/shadowsocks-v1.14.1.${arch}-unknown-linux-gnu.tar.xz"
...
update_sh(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/xxsisis/Shadowsocks-Rust/master/ss-go.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -O ss-go.sh --no-check-certificate https://raw.githubusercontent.com/xxsisis/Shadowsocks-Rust/master/ss-go.sh && chmod +x ss-go.sh
